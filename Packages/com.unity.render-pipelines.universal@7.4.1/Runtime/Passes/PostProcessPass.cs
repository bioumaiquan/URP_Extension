...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        // Misc
        const int k_MaxPyramidSize = 8; // Marked by Bioum : 原始数值为16
        ...
        RenderTextureDescriptor GetStereoCompatibleDescriptor() => GetStereoCompatibleDescriptor(m_Descriptor.width, m_Descriptor.height, m_Descriptor.graphicsFormat, m_Descriptor.depthBufferBits);
        ...
        void SetupBloom(CommandBuffer cmd, int source, Material uberMaterial)
        {
            ...
            float logs = Mathf.Log(s, 2f) + Mathf.Min(m_Bloom.iterationCount.value, 10f) - 10f;
            int iterations = Mathf.FloorToInt(logs);
            int mipCount = Mathf.Clamp(iterations, 1, k_MaxPyramidSize);
            // Marked by Bioum //
