Shader "Hidden/Universal Render Pipeline/Bloom"
{
    ...
    HLSLINCLUDE
        ...
        half4 FragBlurH(Varyings input) : SV_Target
        {
            ...
            // 9-tap gaussian blur on the downsampled source
            half3 c0 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv - float2(texelSize * 3.23076923, 0.0)));
            half3 c1 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv - float2(texelSize * 1.38461538, 0.0)));
            half3 c2 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv                                      ));
            half3 c3 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv + float2(texelSize * 1.38461538, 0.0)));
            half3 c4 = DecodeHDR(SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv + float2(texelSize * 3.23076923, 0.0)));
        ...
        half4 FragDownSample(Varyings input) : SV_Target
        {
            ...
            half4 s;
            float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);
            s =  SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv + d.xy);
            s += SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv + d.zy);
            s += SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv + d.xw);
            s += SAMPLE_TEXTURE2D_X(_MainTex, sampler_LinearClamp, uv + d.zw);
            half3 color = s.rgb * 0.25;
